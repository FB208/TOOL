...
namespace DLL.TOOL
{
    public class ImagePro
    {
        ...
        public static void MakePhoto(string tmp, byte[] er, bool isdeldir)
        {
            lock (o)
            {
                muxConsole.WaitOne();
                try
                {
                    if (isdeldir)
                    {
                        DirFile.DeleteDir(DirFile.GetFoldPath(tmp));
                        DirFile.CreateDir(DirFile.GetFoldPath(tmp));
                    }
                    else
                    {
                        DirFile.DeleteFile(tmp);
                    }
        ...
        public static void MakeThumbPhoto(string originalImagePath, string thumbnailPath, int width, int height, string mode)
        {
            if (originalImagePath.Length > 0)
            {
                ...
                {
                    //DAL.LogTable.Insertlog("原图" + originalImagePath + "缩略" + thumbnailPath, "生成图片异常");
                }
                finally
                {
